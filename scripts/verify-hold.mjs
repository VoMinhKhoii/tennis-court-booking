...
const hold = (date, start, who) =>
	svc.rpc("create_pending_booking", {
		p_type: "adhoc",
		p_customer_name: `${NAME}_${who}`,
		p_zalo_phone: `09900${String(60000 + who).slice(-5)}`,
		p_group_size: 1,
		p_start_time: start,
		p_block_count: 2,
		p_month: null,
...
try {
	...
	log(
		wins.length === courtCount,
		`same-slot stampede: winners == court count (${wins.length}/${courtCount})`,
	);
	log(
		losers.every((r) =>
			/no_court_available|exclusion|overlap|23P01/i.test(r.error.message || r.error.code || ""),
		),
		`all ${losers.length} losers fail cleanly`,
		losers[0]?.error?.message,
	);
	log(
		new Set(wins.map((w) => w.data[0].court_id)).size === wins.length,
		"each winner on a distinct court",
	);
	const occ1 = await activeOcc();
	log(
		occ1.length === courtCount && occ1.every((o) => o.status === "held"),
		`DB holds ${courtCount} held occurrences`,
	);
	...
	const { data: occ2 } = await svc
		...
		.eq("booking_id", bk.id);
	log(
		bk.status === "pending" && occ2.every((o) => o.status === "pending"),
		"finalize: held -> pending (mirrored)",
	);
	...
	await svc
		.from("booking")
		.update({ hold_expires_at: new Date(Date.now() - 60000).toISOString() })
		.eq("reference", eRef);
